...
class _SeedVR2InMemoryDOVERScorer:
    """In-memory DOVER scorer used by Rev 3 four-way analysis.

    This keeps DOVER on the same tensor-backed path as the original Rev 3
    four-way run while the public node carries the full old metric set.
    """

    _evaluator = None
    _evaluator_device = None
    _evaluator_opt = None
    _tensor_registry: dict[str, Any] = {}
    _patched = False
    ...
    def _ensure_video_reader_patched(cls):
        ...
        class _WebPReader:
            def __init__(self, path):
                ...
                self._n = int(getattr(self._im, "n_frames", 1))
            ...
            def __getitem__(self, idx):
                import numpy as np
                import torch
                self._im.seek(int(idx))
                arr = np.asarray(self._im.convert("RGB"), dtype=np.uint8)
                return torch.from_numpy(arr)

        def _dispatch(path, *args, **kwargs):
            p = str(path)
            if p.startswith("tensor://"):
                return _TensorReader(p)
            if p.lower().endswith(".webp"):
                return _WebPReader(p)
    ...
    @classmethod
    def _ensure_evaluator(cls, device: str):
        if cls._evaluator is not None and cls._evaluator_device == device:
            return cls._evaluator, cls._evaluator_opt
        import torch
        import yaml
        cls._ensure_dover_imported()
        ...
        # dover.yml uses relative paths for weights; resolve from DOVER_ROOT.
        weights_path = (DOVER_ROOT / opt["test_load_path"]).resolve()
        ev = DOVER(**opt["model"]["args"]).to(device)
        ev.load_state_dict(torch.load(str(weights_path), map_location=device))
        ev.eval()
        cls._evaluator = ev
        cls._evaluator_device = device
        cls._evaluator_opt = opt
        return ev, opt

    @classmethod
    def _score_frames(cls, frames_nhwc_float01, device: str) -> float:
        """Score one video (NHWC float in [0,1]) via DOVER. Returns the
        fused [0,1] scalar score using the standard val-l1080p sampling
        and the same fuse formula as evaluate_one_video.py."""
        import math
        import torch
        cls._ensure_video_reader_patched()
        ev, opt = cls._ensure_evaluator(device)
        cls._ensure_dover_imported()
        from dover.datasets import dover_datasets as _dd  # type: ignore
        from dover.datasets import UnifiedFrameSampler  # type: ignore

        # Convert to T,H,W,3 uint8 CPU tensor for the registry.
        frames_u8 = (
            frames_nhwc_float01.detach().cpu().clamp(0.0, 1.0) * 255.0
        ).to(dtype=torch.uint8).contiguous()
        key = uuid.uuid4().hex
        cls._tensor_registry[key] = frames_u8
        try:
            ...
            views, _ = _dd.spatial_temporal_view_decomposition(
                f"tensor://{key}", dopt["sample_types"], temporal_samplers
            )
            ...
            with torch.no_grad():
                results = [r.mean().item() for r in ev(views)]
            x = (
            ...
            cls._tensor_registry.pop(key, None)

class SeedVR2FourWayAnalysis(_SeedVR2InMemoryDOVERScorer, SeedVR2EquivalenceAnalysis):
    """Rev 3 four-way analysis.

    Carries forward the old paired-analysis metric set:
      - FR: PSNR, SSIM, LPIPS, DISTS against the HQ reference
      - NR: NIQE, MUSIQ, CLIP-IQA, DOVER fused

    Adds floor-normalized raw percent scores per metric:
        abs(candidate - floor) / abs(reference - floor)
    """

    FR_REFERENCE_ANCHORS: dict[str, Any] = {
        "psnr": "inf",
        "ssim": 1.0,
        "lpips": 0.0,
        "dists": 0.0,
    ...
    RETURN_TYPES = ("STRING", "STRING", "STRING")
    RETURN_NAMES = ("metrics_json", "artifact_path", "raw_percent_json")
    FUNCTION = "execute"
    CATEGORY = "SeedVR2/analysis"
    OUTPUT_NODE = True
    ...
    @staticmethod
    def _reference_fr_metric_block(metric: str, frame_count: int) -> dict[str, Any]:
        anchor = SeedVR2FourWayAnalysis.FR_REFERENCE_ANCHORS[metric]
        if isinstance(anchor, str):
            return {
                ...
                "max": anchor,
                "per_frame": [],
                "anchor_note": "identical-reference PSNR is infinite",
            }
        return _aggregate([float(anchor)] * frame_count)

    @staticmethod
    def _metric_mean(block: dict[str, Any] | None) -> Any:
        if not block:
            return None
    ...
    def _metric_values(
        ...
        for metric, family in cls.METRIC_ROWS:
            if family == "fr" and metric not in enabled_fr:
                continue
            if family == "nr" and metric not in enabled_nr:
                continue
            values[metric] = {
                "reference": cls._metric_mean(metrics["reference"][family].get(metric)),
                "floor_esrgan": cls._metric_mean(metrics["floor_esrgan"][family].get(metric)),
                "numz": cls._metric_mean(metrics["numz"][family].get(metric)),
                "native": cls._metric_mean(metrics["native"][family].get(metric)),
            }
        return values
    ...
    def _normalized_scores(
        ...
        raw_percent: dict[str, dict[str, Any]] = {}
        status: dict[str, dict[str, Any]] = {}
        for metric, values in metric_values.items():
            reference = cls._coerce_finite_float(values.get("reference"))
            floor = cls._coerce_finite_float(values.get("floor_esrgan"))
            numz = cls._coerce_finite_float(values.get("numz"))
            native = cls._coerce_finite_float(values.get("native"))
            normalized[metric] = {"reference": None, "numz": None, "native": None}
            raw_percent[metric] = {"numz": None, "native": None}
            if reference is None or floor is None:
                status[metric] = {
                    "denominator_unstable": True,
                    "reason": "reference or floor anchor is not finite",
                }
                continue
            reference_normal = abs(reference - floor)
            normalized[metric]["reference"] = reference_normal
            if reference_normal < 1e-9:
                status[metric] = {
                    "denominator_unstable": True,
                    "reason": "reference and floor anchors are equal",
                }
                continue
            if numz is not None:
                numz_normal = abs(numz - floor)
                normalized[metric]["numz"] = numz_normal
                raw_percent[metric]["numz"] = numz_normal / reference_normal
            if native is not None:
                native_normal = abs(native - floor)
                normalized[metric]["native"] = native_normal
                raw_percent[metric]["native"] = native_normal / reference_normal
            status[metric] = {"denominator_unstable": False, "reason": None}
        return normalized, raw_percent, status

    @staticmethod
    def _format_value(value: Any, width: int = 14, precision: int = 4) -> str:
        if value is None:
            return f"{'--':>{width}}"
    ...
    @classmethod
    def _render_fourway_text_table(cls, artifact: dict[str, Any]) -> str:
        videos = artifact.get("videos", {}) or {}
        metric_values = artifact.get("metric_values", {}) or {}
        raw_percent = artifact.get("raw_percent_score", {}) or {}
        ...
        lines = []
        lines.append("=" * 112)
        lines.append("SeedVR2 Four-Way Analysis  -  reference / floor / numz / native")
        lines.append("=" * 112)
        lines.append(f"reference (HQ):       {_video_label('reference')}")
        lines.append(f"floor (ESRGAN):       {_video_label('floor_esrgan')}")
        lines.append(f"numz candidate:       {_video_label('numz')}")
        lines.append(f"native candidate:     {_video_label('native')}")
        lines.append("-" * 112)
        lines.append(
            f"{'metric':<14} {'reference':>14} {'floor':>14} "
            f"{'numz':>14} {'native':>14} {'numz_raw':>14} {'native_raw':>14}"
        )
        lines.append("-" * 112)
        for metric, _family in cls.METRIC_ROWS:
            values = metric_values.get(metric)
            if not values:
                continue
            raw = raw_percent.get(metric, {}) or {}
            lines.append(
                f"{metric:<14} "
                f"{cls._format_value(values.get('reference'))} "
                f"{cls._format_value(values.get('floor_esrgan'))} "
                f"{cls._format_value(values.get('numz'))} "
                f"{cls._format_value(values.get('native'))} "
                f"{cls._format_value(raw.get('numz'))} "
                f"{cls._format_value(raw.get('native'))}"
            )
        lines.append("=" * 112)
        return "\n".join(lines) + "\n"

    @staticmethod
    def _artifact_path(output_directory: str, output_filename: str) -> Path:
        if output_directory.strip():
            artifact_dir = Path(output_directory).expanduser().resolve()
        else:
            artifact_dir = SeedVR2FourWayAnalysis._artifact_dir()
        artifact_dir.mkdir(parents=True, exist_ok=True)
        if output_filename.strip():
            fname = output_filename.strip()
            if not fname.lower().endswith(".json"):
            ...
            fname = f"seedvr2_fourway_{uuid.uuid4().hex}.json"
    ...
    def execute(
        ...
        artifact_path = self._artifact_path(output_directory, output_filename)

        ref_frames, ref_fps = _frames_from_video(video_reference)
        floor_frames, floor_fps = _frames_from_video(video_floor)
        numz_frames, numz_fps = _frames_from_video(video_numz)
        native_frames, native_fps = _frames_from_video(video_native)

        video_meta = {
            "reference": self._alignment_metadata(ref_frames, ref_fps),
            "floor_esrgan": self._alignment_metadata(floor_frames, floor_fps),
            "numz": self._alignment_metadata(numz_frames, numz_fps),
            "native": self._alignment_metadata(native_frames, native_fps),
        }
        self._assert_alignment_pair(video_meta["reference"], video_meta["floor_esrgan"], "reference", "floor_esrgan")
        self._assert_alignment_pair(video_meta["reference"], video_meta["numz"], "reference", "numz")
        self._assert_alignment_pair(video_meta["reference"], video_meta["native"], "reference", "native")

        backend = self._metric_backend or SeedVR2MetricBackend(lpips_backbone=lpips_backbone)
        videos = {
            "reference": (ref_frames, ref_fps),
            "floor_esrgan": (floor_frames, floor_fps),
            "numz": (numz_frames, numz_fps),
            "native": (native_frames, native_fps),
        }
        metrics: dict[str, dict[str, dict[str, dict[str, Any]]]] = {
            side: {"fr": {}, "nr": {}} for side in videos
        }

        for side, (frames, frame_rate) in videos.items():
            if enabled_nr:
                print(f"[SeedVR2FourWayAnalysis] scoring NR metrics for {side}", flush=True)
                pyiqa_nr = set(enabled_nr)
                use_direct_dover = "dover_fused" in pyiqa_nr
                pyiqa_nr.discard("dover_fused")
                per_frame_nr = self._per_frame_nr(backend, frames, frame_rate, pyiqa_nr)
                if use_direct_dover:
                    prev_cwd = os.getcwd()
                    try:
                        os.chdir(str(DOVER_ROOT))
                        per_frame_nr["dover_fused"] = [
                            float(self._score_frames(frames, device))
                        ]
                    finally:
                        os.chdir(prev_cwd)
                metrics[side]["nr"] = {
                    name: self._aggregate_block(values) for name, values in per_frame_nr.items()
                }

        if enabled_fr:
            ref_count = int(ref_frames.shape[0])
            metrics["reference"]["fr"] = {
                name: self._reference_fr_metric_block(name, ref_count)
                for name in sorted(enabled_fr)
            }
            for side, frames in (
                ("floor_esrgan", floor_frames),
                ("numz", numz_frames),
                ("native", native_frames),
            ):
                print(f"[SeedVR2FourWayAnalysis] scoring FR metrics for {side}", flush=True)
                per_frame_fr = self._per_frame_fr(backend, frames, ref_frames, enabled_fr)
                metrics[side]["fr"] = {
                    name: self._aggregate_block(values) for name, values in per_frame_fr.items()
                }
                if "lpips" in metrics[side]["fr"]:
                    metrics[side]["fr"]["lpips"]["backbone"] = backend._lpips_backbone
            if "lpips" in metrics["reference"]["fr"]:
                metrics["reference"]["fr"]["lpips"]["backbone"] = backend._lpips_backbone

        metric_values = self._metric_values(metrics, enabled_fr, enabled_nr)
        normalized, raw_percent, normalization_status = self._normalized_scores(metric_values)
        raw_percent_json = json.dumps(raw_percent, indent=2, sort_keys=True)

        artifact = {
            "schema_version": "rev3.1",
            "node": "SeedVR2FourWayAnalysis",
            "formula": "raw_percent_score = abs(candidate - floor) / abs(reference - floor)",
            "inputs": {
                "video_reference": (
                    repr(type(video_reference).__name__) if _is_video_object(video_reference)
                    else str(video_reference)
                ),
                "video_floor": (
                    repr(type(video_floor).__name__) if _is_video_object(video_floor)
                    else str(video_floor)
                ),
                "video_numz": (
                    repr(type(video_numz).__name__) if _is_video_object(video_numz)
                    else str(video_numz)
                ),
                "video_native": (
                    repr(type(video_native).__name__) if _is_video_object(video_native)
                    else str(video_native)
                ),
            },
            "videos": video_meta,
            "enabled_metrics": {
        ...
        metrics_json = json.dumps(artifact, indent=2, sort_keys=True)
        artifact_path.write_text(metrics_json, encoding="utf-8")
        print(self._render_fourway_text_table(artifact), flush=True)
        return (metrics_json, str(artifact_path), raw_percent_json)


SeedVR2DOVERFourWay = SeedVR2FourWayAnalysis
